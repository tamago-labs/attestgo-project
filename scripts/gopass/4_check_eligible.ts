/**
 * 4_check_eligible.ts — view isEligible + record
...
async function main() {
  ...
  const rec: any = await (m as any).getRecord(wallet);
  ...
  const ok = await (m as any).isEligible(wallet, rule);
  console.log(`isEligible (US|SG, min 10): ${ok ? '✅ Ready to Receive' : '❌ Not eligible / Syncing...'}`);
}
