...
Worker `3_worker_sync` is Advance-style: off-chain `getRecord` + `continuityLen=2` check, then `mirror.markVerified` (no on-chain `0x0FD2` decode). Fallback `syncPass` with proof via `0x0FD2` stays as `GOPassMirror.syncPass` if worker down.
