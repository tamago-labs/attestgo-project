...
export default function ButtonGlow({ children, href, className }: { children: React.ReactNode; href: string; className?: string }) {
  ...
  return (
    <a ref={containerRef} href={href} className="relative inline-flex items-center group">
      <span className={`relative z-10 inline-flex items-center rounded-lg border border-border text-sm font-medium text-white group-hover:bg-panel transition-colors ${className ?? ""}`}>
        {children}
      </span>
